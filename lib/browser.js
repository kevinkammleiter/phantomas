...
this.injectLocalStorage = async function (page, storage) {
  if (!page || !storage) {
    return;
  }
...
this.injectSessionStorage = async function (page, storage) {
  if (!page || !storage) {
    return;
  }
  page.evaluateOnNewDocument((storage) => {
...
Browser.prototype.init = async (phantomasOptions) => {
  ...
  try {
    ...
    if (phantomasOptions.localStorage) {
        this.injectLocalStorage(this.page, phantomasOptions.localStorage);
    }
    if (phantomasOptions.sessionStorage) {
        this.injectSessionStorage(this.page, phantomasOptions.sessionStorage);
    }
